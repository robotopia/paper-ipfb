...
\begin{description}
    ...
    \item[``Filter error'':] the error caused by the fact that the synthesis filter ($f[n]$) does not exactly complement the analysis filter ($h[n]$) in the sense described by Eq. \eqref{eqn:invertibility_condition}.
\end{description}
...
Although the power in the echoes is small compared to the central impulse (at roughly the $-10\,$dB level), the echoes always appear at the same locations relative to it.
...
Thus, any structure seen on this time scale must therefore be regarded with suspicion.

As a further verification, we also made use of a non-standard observing mode to record a small amount of simultaneous coarse and fine channel data (i.e. both before and after the fine PFB analysis filter stage).
The fine channels from one polarisation of a single tile were extracted and subjected to the synthesis filter to reconstruct the $1.28\,$MHz coarse channel time series, which could then be compared directly with the original data.
Although not as telling as an impulse response test, the close similarity of the reconstructed (white) noise with the original (see Fig. \ref{fig:compare_timeseries}) is indicative of the fidelity of the analysis-synthesis filter pair.

Note that in this test, the FPGA-based fine PFB was used; thus, the errors shown in Fig. \ref{fig:compare_timeseries} do not contain any replication error.
However, the synthesis filter used in production uses an arbitrary scaling factor in order to make better use of the dynamic range afforded by VDIF's (8+8)-bit depth.
...
Pilot observations of three pulsars are presented here to validate the synthesis filter as a useful tool for undertaking high time resolution studies of pulsars: MSPs \psrkaurJ{} and \psrbhatJ{}, and the bright PSR \psrslowB{}.
...
Finally, recent independent follow-up observations made by some of the authors using Band 3 ($250$ to $500\,$MHz) of the upgraded Giant Metrewave Radio Telescope (uGMRT) have confirmed that at least the innermost component is real (Kaur et al., in prep).
...
PSR \psrbhatJ{} is a nearby MSP (distance $d \approx 157\,$pc; period $P \approx 5.76\,$ms) and an established target for pulsar timing array (PTA) applications.
...
\psrslowB{} is a bright, slow ($P = 0.253\,$s) pulsar known to exhibit microstructure \citep{Popov2002,Kuzmin2003}.
...
Despite the proximity of this timescale to the temporal ``echoes'' induced by the synthesis filter ($50\mu$s), we were able to clearly identify the ($\sim 60\mu$s) microstructure reported by \citet{Kuzmin2003} at $102.5\,$MHz.
...
The MWA-VCS telescope system outputs dynamic spectra with a frequency resolution of $10\,$kHz and a time resolution of $100\,\mu$s, which is suitable for many pulsar applications, as demonstrated in several pulsar science papers published to date.
...
Although the reconstruction is not perfect, the introduced errors are quantified (up to $-10\,$dB in certain samples).
...
We have further verified our system by observing PSRs \psrkaurJ{}, \psrbhatJ{}, and \psrslowB{}, all of which are known to exhibit structures on microsecond timescales.
In each case, our results have been shown to be consistent with observations at higher radio frequencies.
...
The advent of the VCS high time resolution observing mode thus distinguishes the MWA as a premier low-frequency instrument for studying pulsars and other transients at microsecond precision, with important implications for studies of the pulsar emission mechanism, the characterisation of the ISM in the ongoing search for gravitational waves, and others.
