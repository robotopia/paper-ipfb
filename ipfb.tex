...
Minimising the loss of S/N is equivalent to solving Eq. \eqref{eqn:invertibility_condition} in a least squares sense.
...
\begin{equation}
    ...
    \begin{bmatrix}
       ...
       \vdots & & \ddots & \vdots \\
       h_0[-n] & h_1[-n] & \cdots & h_{P-1}[-n] \\
       \vdots & & \ddots & \vdots \\
       0 & \cdots & h_0[-n] & h_1[-n] \\
       0 & \cdots & 0 & h_0[-n]
    \end{bmatrix},
\end{equation}
with row number $i = P - 1$ (in $H^{(n)}$ and $D$) corresponding to the $s = 0$ term.
...
With this notation, the reconstruction error can be more simply expressed
\begin{equation}
    \frac{[\text{S/N}]_\text{recon}}{[\text{S/N}]_\text{orig}}
...
The performance of both the mirror filter and the set of optimal solutions (for different choices of $P^\prime$) can be evaluated straightforwardly using Eq. \eqref{eqn:snr_short}.
